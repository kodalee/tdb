...
class SourceQuery {
	...
	public function getInfos() {
		...
		trigger_error('Unknown server type: '. $protocol, E_USER_NOTICE);
		return false;
	}
	...
	public function getPlayers() {
		...
		for ($i = 0; isset($infos[$i + 1]); $i = $j + 9) {
            ...
            // lets do the above without using eval
            $time = trim(chunk_split($infos[$j + 5] . $infos[$j + 6] . $infos[$j + 7] . $infos[$j + 8], 2, ''), "\x");
